...
GOBGPD_CFG="$ROOT_DIR/scripts/interop/gobgpd.toml"
FOCL_CFG="$ROOT_DIR/scripts/interop/focl-interop.toml"
FOCL_SOCKET="$ROOT_DIR/focld-interop.sock"
GOBGPD_LOG="$RUNTIME_DIR/gobgpd.log"
FOCLD_LOG="$RUNTIME_DIR/focld.log"
...
if ! wait_for_established; then
  echo "BGP session did not reach Established" >&2
  echo "--- gobgpd.log ---" >&2
...
echo "Interop OK: Established + prefix 203.0.113.0/24 received by GoBGP"
